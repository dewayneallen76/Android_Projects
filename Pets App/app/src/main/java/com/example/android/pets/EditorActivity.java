...
public class EditorActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
     ...
     * the view, and we change the @param mPetHasChanged boolean to true.
     */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mCurrentPetUri == null) {
            ...
            // Otherwise this is an exisiting pet, so change the app bar to say "Edit Pet".
        ...
        // Setup OnTouchListeners on all of the input fields, so we can determine if the user has
    ...
    private void savePet() {
        ...
        // If the weight is not provided by the user, don't try to parse the string into and integer
        // value. Use 0 by default.
        int weight = 0;
        ...
        if (mCurrentPetUri == null) {
            // This is a NEW pet, so insert a new pet into the provder, returning the content
            // URI for the new pet.
            Uri newUri = getContentResolver().insert(PetContract.PetEntry.CONTENT_URI, values);

            // Show a toast message depending on whether or not the insertion was successful.
            if (newUri == null) {
                // If the new content URI is null, then there is an error with insertion.
                Toast.makeText(this, getString(R.string.editor_insert_pet_failed),
                        Toast.LENGTH_SHORT).show();
            } else {
                // Otherwise the insertion was successful and we can display message
                Toast.makeText(this, getString(R.string.editor_insert_pet_successful),
                        Toast.LENGTH_SHORT).show();
            }
        } else {
            // Otherwise this is an EXISTING pet, so update the pet with content URI: mCurrentPetUri
            // and pass in the Content Values. Pass in null for the selection and selection args
            // because mCurrentPetUri will already identify the correct row in the database that we
            // want to modify.
            ...
            if (rowsAffected == 0) {
                ...
                // Otherwise the the update was successful and we can display a toast.
     ...
     * (some menue items can be hidden or made visible).
     */
    @Override
    public boolean onPrepareOptionsMenu(Menu menu) {
        super.onPrepareOptionsMenu(menu);
        // If this is a new post, hide the "Delete" menu item.
        if (mCurrentPetUri == null) {
    ...
    public void onBackPressed() {
        // If the pet hasn't changed, continue with handling back button request.
        if (!mPetHasChanged) {
            super.onBackPressed();
            return;
        }
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        if (cursor.moveToFirst()) {
            ...
            // Then call setSelection() so that the option is displayed on the screen as the current
            // selection.
            switch (gender) {
    ...
    public void onLoaderReset(Loader<Cursor> loader) {
        // If the loader is invalidated, clear out all of the data from the input fields.
    ...
    /**
     * Show a dialog box that warns the user there are unsaved changes that will be lost if they
     * continue.
     *
    ...
    private void showUnsavedChangesDialog(
            DialogInterface.OnClickListener discardButtonClickListener) {
        // Create an AlertDialog.Builder and set the message, and click listeners for the negative
        // and positive buttons in the dialog
    ...
    private void deletePet() {
        // Only perform the delete is this is an existing pet.
        if (mCurrentPetUri != null) {
